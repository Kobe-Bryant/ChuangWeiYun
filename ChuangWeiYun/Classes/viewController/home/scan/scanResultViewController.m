...
- (void)viewDidLoad
{
    ...
    [cancelItem release], cancelItem = nil;
...
-(void) viewDidAppear:(BOOL)animated{
    ...
    if (dataFromScan) 
    {
        if (self.resultString.length > 0 && self.info.length > 0)
        {
            ...
            scanHistoryMod.where = [NSString stringWithFormat:@"type = '%@' and info = '%@' and result = '%@'",self.typeString,self.info,resultString];
            NSArray *dbArr = [scanHistoryMod getList];
            if ([dbArr count] > 0) {
                [scanHistoryMod deleteDBdata];
            }
            scanHistoryMod.where = nil;
