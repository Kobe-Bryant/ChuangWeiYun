...
+ (NSString *)defaultUserAgentString
{
	...
	NSString *appNames = [bundle objectForInfoDictionaryKey:@"CFBundleDisplayName"];
	if (!appNames) {
		appNames = [bundle objectForInfoDictionaryKey:@"CFBundleName"];
	}
	...
	return [NSString stringWithFormat:@"%@ %@ (%@; %@ %@; %@)", appNames, appVersion, deviceName, OSName, OSVersion, locale];
}
