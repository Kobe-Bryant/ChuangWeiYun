...
+ (void) removeFileDB:(NSString *)filename
{
	...
	if (filename.length > 4) {
		...
		if ([self isExistsFile:[self getFileDBPath:filename]]) {
			if ([filemanage removeItemAtPath:[self getFileDBPath:filename] error:&error] != YES)
			{
...
+ (NSString *) readLocalFileData:(NSString *)filename {
    ...
    NSString *gData2 = [[NSString alloc] initWithData:[reader subdataWithRange:NSMakeRange(0, [temp length])]
