...
@implementation PopCancelOrderView
@synthesize delegate;
...
- (id)init
{
    ...
    if (self) {
        ...
        UILabel *verticalLine=[[UILabel alloc]initWithFrame:CGRectMake(width/2-0.4, height, 1.18, kcontorlHeight-height)];
        ...
        [noBtn setTitle:@"不好" forState:UIControlStateNormal];
...
//确定取消预订
-(void)okCancelClick{
    if ([self.delegate respondsToSelector:@selector(OKCancel)]) {
        [self.delegate performSelector:@selector(OKCancel)];
    }
