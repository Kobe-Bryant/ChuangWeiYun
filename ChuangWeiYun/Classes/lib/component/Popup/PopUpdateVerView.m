...
@implementation PopUpdateVerView
...
- (id)init:(NSString *)marked andBtnTitle:(NSString *)yesTitle andTitle:(NSString *)noTitle
{
    ...
    if (self) {
        ...
        label.textColor = [UIColor colorWithRed:76/255.0 green:76/255.0 blue:76/255.0 alpha:1];
...
-(void)okCancelClick{
    if ([self.delegate respondsToSelector:@selector(OKCheckUpdate)]) {
        [self.delegate performSelector:@selector(OKCheckUpdate)];
    }
